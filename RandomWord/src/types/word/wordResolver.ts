...
const getRandomWord = (args: { language: string }): string | undefined => {
    const lines = fs.readFileSync(
        `${process.cwd()}/src/types/word/data/${args.language}.txt`,
        'utf8'
    );
    const words = lines.split('\n');
    const word = words[Math.floor(Math.random() * words.length)].replace('\r', '');
    return word;
};

const createWord = (args: { input: { word: string; language: string } }): string => {
    fs.writeFile(
        `${process.cwd()}/src/types/word/data/${args.input.language}.txt`,
        args.input.word,
        (err) => {
            if (err) {
                console.error(err);
